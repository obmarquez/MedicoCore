...
namespace MedicoCore.Data
{
    public class DBOperaciones
    {
        ...
        public IEnumerable<T> Getunparam<T>(string sp, int opcion)
        {
            ...
            {
                cnn = new DBConnection().Open();
                result = cnn.Query<T>(sp, new { opcion }, null, true, null, CommandType.StoredProcedure).ToList();
            }
            catch (Exception)
            {
        ...
        public IEnumerable<T> Getdosparam<T>(string sp, int opcion, int id)
        {
            ...
            {
                cnn = new DBConnection().Open();
                result = cnn.Query<T>(sp, new { opcion, id }, null, true, null, CommandType.StoredProcedure).ToList();
            }
            catch (Exception)
            {
